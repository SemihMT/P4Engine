...
void dae::BubbleEventHandlerComponent::OnNotify(Event event, const EventData& data)
{
	...
	case Event::Bubble_HitEnemy:
	{
		...
		if (enemyThatGotHit->GetName() == "ZenChan")
		{
			state = std::make_unique<ZenChanVulnerableState>(enemyThatGotHit);
		}
		else
		{
			state = std::make_unique<MaitaVulnerableState>(enemyThatGotHit);
		}
