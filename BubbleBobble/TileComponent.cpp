...
dae::TileComponent::TileComponent(GameObject* owner, int tileIdx, ColliderType type) : BaseComponent(owner), m_transform{ owner->GetTransform() }, m_tileRow{ tileIdx }
{
	owner->SetName("Tile");
	...
	owner->AddComponent<ColliderComponent>(m_tileSize, type);
